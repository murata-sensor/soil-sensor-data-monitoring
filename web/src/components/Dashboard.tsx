...
import {
  ...
  type PanelSettings, type UserChartPanel, type UserSettings,
  dateRangeToMs, loadSettings, parseTs, saveSettings,
} from "../settings";
import type { LayoutConfig, TextPanelConfig } from "../layoutConfig";
...
export default function Dashboard() {
  ...
  const visibleCustomLayout = useMemo(() => {
    ...
    // Merge user-created text panels and chart panels
    const userTextPanels = settings?.userTextPanels ?? [];
    const userChartPanels: import("../layoutConfig").LayoutPanel[] = (settings?.userChartPanels ?? []).map((uc) => ({
      type: "chart" as const,
      id: uc.id,
      title: uc.title,
      metric: uc.metric,
  ...
  const handleResetLayout = useCallback(() => {
    updateSettings({ layout: null, userTextPanels: [], userChartPanels: [], textOverrides: {} });
    setCustomLayoutResetKey((k) => k + 1);
  }, [updateSettings]);

  const handleTextEdit = useCallback((panelId: string, content: string) => {
    if (!settings) return;
    updateSettings({ textOverrides: { ...settings.textOverrides, [panelId]: content } });
  }, [settings, updateSettings]);

  const handleAddTextPanel = useCallback((panel: TextPanelConfig) => {
    if (!settings) return;
    updateSettings({ userTextPanels: [...(settings.userTextPanels ?? []), panel] });
    setCustomLayoutResetKey((k) => k + 1);
  }, [settings, updateSettings]);

  const handleAddChartPanel = useCallback((panel: UserChartPanel) => {
    if (!settings) return;
    updateSettings({ userChartPanels: [...(settings.userChartPanels ?? []), panel] });
    setCustomLayoutResetKey((k) => k + 1);
  }, [settings, updateSettings]);

  const handleRemoveUserPanel = useCallback((panelId: string) => {
    if (!settings) return;
    updateSettings({
      userTextPanels: (settings.userTextPanels ?? []).filter((p) => p.id !== panelId),
      userChartPanels: (settings.userChartPanels ?? []).filter((p) => p.id !== panelId),
      textOverrides: Object.fromEntries(
        Object.entries(settings.textOverrides ?? {}).filter(([k]) => k !== panelId),
      ),
    });
    setCustomLayoutResetKey((k) => k + 1);
  }, [settings, updateSettings]);
...
function GridContainer({ layout, panels, filteredRows, visibleEvents, colors, settings, showEventLabels, chartBgColor, onLayoutChange, userTextPanels, textOverrides, onTextEdit, onRemoveUserPanel, readOnly }: {
  ...
  // Build merged layout: chart panels + user text/chart panels
  const mergedLayout = useMemo(() => {
    const textLayouts = userTextPanels.map((p) => ({
      i: p.id,
      x: p.position.x,
      y: p.position.y,
      w: p.position.w,
    ...
    return [...layout, ...textLayouts];
  }, [layout, userTextPanels]);
    ...
    <div ref={containerRef as React.Ref<HTMLDivElement>}>
      {width > 0 && (
        ...
        >
          ...
          {userTextPanels.map((tp) => (
            <div key={tp.id}>
              <NormalTextPanel
                panel={tp}
                overrideContent={textOverrides[tp.id]}
                onEdit={onTextEdit}
...
// ─── Metric label map for chart creation ────────────────────────────────────

const METRIC_OPTIONS: { value: Metric; label: string }[] = [
  { value: "vwc_pct", label: "VWC (体積含水率 %)" },
  { value: "ec_bulk_dsm", label: "Bulk EC (dS/m)" },
  { value: "ec_pore_dsm", label: "Pore EC (dS/m)" },
  { value: "temperature_c", label: "土壌温度 (℃)" },
  { value: "battery_v", label: "バッテリー電圧 (V)" },
  { value: "battery_pct", label: "バッテリー (%)" },
  { value: "rssi_dbm", label: "RSSI (dBm)" },
  { value: "air_temp_c", label: "外気温 (℃)" },
  { value: "precip_1h_mm", label: "降水量 (mm/h)" },
  { value: "sunshine_1h_h", label: "日照時間 (h)" },
  { value: "vwc_rock_pct", label: "VWC ロックウール (%)" },
  { value: "vwc_coco_pct", label: "VWC ココピート (%)" },
  { value: "ec_pore_coco_dsm", label: "Pore EC ココピート (dS/m)" },
];

// ─── Add Panel Toolbar ──────────────────────────────────────────────────────

function AddPanelToolbar({ onAddText, onAddChart, existingPanels, cols }: {
  onAddText: (panel: TextPanelConfig) => void;
  onAddChart: (panel: UserChartPanel) => void;
  existingPanels: number;
  cols: number;
}) {
  const [showChartForm, setShowChartForm] = useState(false);
  const [chartTitle, setChartTitle] = useState("");
  const [chartMetric, setChartMetric] = useState<Metric>("vwc_pct");

  const handleAddText = () => {
    const id = `user-text-${Date.now()}`;
    const y = Math.max(0, existingPanels * 4);
    onAddText({
      type: "text",
      id,
      content: "テキストを入力",
      fontSize: "1rem",
      align: "left",
      position: { x: 0, y, w: Math.min(4, cols), h: 1 },
    });
  };

  const handleAddChart = () => {
    if (!chartTitle.trim()) return;
    const id = `user-chart-${Date.now()}`;
    const y = Math.max(0, existingPanels * 4);
    onAddChart({
      id,
      ...
      {showChartForm ? (
        <div className="flex items-center gap-2 flex-wrap">
          <input
            ...
            maxLength={60}
            onKeyDown={(e) => { if (e.key === "Enter") handleAddChart(); if (e.key === "Escape") setShowChartForm(false); }}
            autoFocus
          />
          <select
            value={chartMetric}
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-emerald-600 text-white text-xs hover:bg-emerald-700 disabled:opacity-40"
          >
            追加
          </button>
          <button
    ...
    return (
      ...
      >
        <textarea
          ...
          className="flex-1 resize-none rounded border border-indigo-400 px-2 py-1 outline-none focus:border-indigo-300 text-sm"
        />
        <div className="flex gap-1 justify-end mt-1">
          <button onClick={handleSave} className="px-2 py-0.5 rounded bg-indigo-600 text-white text-[10px]">保存</button>
          <button onClick={() => { setDraft(content); setEditing(false); }} className="px-2 py-0.5 rounded bg-slate-200 text-[10px]">キャンセル</button>
        </div>
      </section>
    );
  }
...
// ─── User Panel Wrapper (delete button for user-created chart panels) ───────

function UserPanelWrapper({ panelId, onRemove, children }: {
  panelId: string;
