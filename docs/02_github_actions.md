...
- センサ異常値や電池残量低下が検知された場合は `alerts` シートに行が追加される（メール通知は [04_gas_setup.md](04_gas_setup.md) のアラート設定が必要）
- `DRIVE_BACKUP_FOLDER_ID` を設定した場合は、Drive フォルダにも CSV が積み上がる
- 以降は毎日 **09:30 JST**（UTC 00:30）に自動実行される

---

## 4. 取り込んだデータをダッシュボードで見られるようにする
...
| sourceId | displayName | schemaType | spreadsheetId | sheetName | headerRow | tz | accessMode | enabled | notes |
|----------|-------------|------------|---------------|-----------|-----------|-----|------------|---------|-------|
| `src-remote-a` | `A圃場` | `remote-ftp` | （`FTP_SPREADSHEET_ID` と同じ） | `sensor_9am` | `1` | `Asia/Tokyo` | `direct` | `TRUE` | |
