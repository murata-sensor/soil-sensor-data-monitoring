...
| `FTP_SPREADSHEET_ID` | [`01_google_setup.md`](01_google_setup.md) 手順 1-5 で作成した **リモートデータ用** スプレッドシートの ID |
...
| Name | 値の例 | 意味 |
|------|--------|------|
| `SITE_ID` | `site-a` | リモートデータの `siteId` デフォルト。Registry の `sources` 行で個別指定がなければこの値が使われる |
...
4. ブランチが `main` であることを確認し、「Run workflow」を押す
5. 数秒待つとリストに **黄色い丸** の実行が表示される。クリックして詳細を開く
6. 中の「**ingest**」ジョブをクリック → 各ステップが緑チェックになれば成功
7. 失敗時は赤い ✕ がついたステップを展開すると Python のエラーログが見える

成功すると:

- FTP用スプレッドシートの `sensor_raw` / `sensor_9am` シートに新しい行が追加される
- `DRIVE_BACKUP_FOLDER_ID` を設定した場合は、Drive フォルダにも CSV が積み上がる
- 以降は毎日 **04:00 JST**（UTC 19:00）に自動実行される
